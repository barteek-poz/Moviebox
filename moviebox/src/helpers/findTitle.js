export const findTitle = async (media, page) => {
  const options = {
    method: "GET",
    headers: {
  ...
  const url = `https://api.themoviedb.org/3/trending/${media}/week?language=en-US&page=${page}`;
  ...
  const finalTitle = titleList[Math.floor(Math.random() * titleList.length)]
